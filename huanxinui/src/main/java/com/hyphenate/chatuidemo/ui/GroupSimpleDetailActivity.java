...
public class GroupSimpleDetailActivity extends BaseActivity {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					// TODO: 2016/12/21
					//获得群主信息
					MyParseManager.getInstance().asyncGetUserInfo(group.getOwner(), new EMValueCallBack<EaseUser>() {
						@Override
						public void onSuccess(final EaseUser easeUser) {
							runOnUiThread(new Runnable() {
								public void run() {
									showGroupDetail(easeUser);
								}
							});
						}
						@Override
	...
	public void addToGroup(View view){
     ...
     private void showGroupDetail(EaseUser easeUser) {
         progressBar.setVisibility(View.INVISIBLE);
...
//         tv_admin.setText(group.getOwner());
         tv_admin.setText(easeUser==null?group.getOwner():easeUser.getNickname());
         tv_introduction.setText(group.getDescription());

		 if(easeUser!=null){
			 Glide.with(this).load(easeUser.getAvatarUrl()).into((ImageView)findViewById(R.id.avatar));
		 }
     }
