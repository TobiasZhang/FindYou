...
//				holder.name.setText(msg.getFrom());
				holder.name.setText("群信息");

				Glide.with(context).load(msg.getGroupInviterAvantar()).into(holder.avator);
			} else{
				holder.groupContainer.setVisibility(View.GONE);
				// TODO: 2016/12/21
				//申请人姓名，头像
				holder.name.setText(msg.getTruename());
				Glide.with(context).load(msg.getUserAvantar()).into(holder.avator);
			}
			...
			if (msg.getStatus() == InviteMesageStatus.BEAGREED) {
			    ...
			    String str = /*msg.getGroupInviter()*/msg.getGroupInviterTruename() + str9 + msg.getGroupName();
                holder.status.setText(str);
                holder.status.setBackgroundDrawable(null);
                holder.status.setEnabled(false);
            } else if(msg.getStatus() == InviteMesageStatus.GROUPINVITATION_DECLINED){
                String str = /*msg.getGroupInviter()*/msg.getGroupInviterTruename() + str10 + msg.getGroupName();
                holder.status.setText(str);
                holder.status.setBackgroundDrawable(null);
                holder.status.setEnabled(false);
            }
