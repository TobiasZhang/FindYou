...
public class DemoDBManager {
    ...
    synchronized public void saveContactList(List<EaseUser> contactList) {
        ...
        if (db.isOpen()) {
            ...
            for (EaseUser user : contactList) {
                ...
                System.out.println(user.getUsername()+"--"+user.getNickname()+"--"+user.getAvatar()+"----saveContactList--------");
            }

            System.out.println("----saveContactList---finish-----");
        }
    }
     ...
     * Save Robot list
     */
	synchronized public void saveRobotList(List<EaseUser> robotList) {
		SQLiteDatabase db = dbHelper.getWritableDatabase();
		if (db.isOpen()) {
			db.delete(UserDao.ROBOT_TABLE_NAME, null, null);
			for (EaseUser item : robotList) {
				ContentValues values = new ContentValues();
